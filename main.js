...
function addWatermark(canvas, text) {
  ...
  tempCtx.font = "bold 16px verdana";
...
function onTelegramAuth(user) {
  ...
  request.send();
  tglogin.setAttribute('class', 'invisible');
  tglogout.innerHTML = tglogout.innerHTML = localStorage.getItem('lang') === 'en' ? `<p>You logged in as <em>${localStorage.getItem("name")}</em></p><p>Now you can save your tabs to <em>telegram</em>.</p><input type="button" id="btnUnsubscribe" value="Log out" onclick="unsubscribe()">` : `<p>Ви увійшли як <em>${localStorage.getItem("name")}</em></p><p>Тепер Ви можете зберігати аплікатурні схеми до окремого чату у <em>telegram</em>.</p><input type="button" id="btnUnsubscribe" value="Вийти" onclick="unsubscribe()">`;
  tglogout.setAttribute('class', '');
}
